...
draw_frame(struct client_state *state)
{
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            data[y * width + x] = row_pointers[y][x];
            /*
            if ((x + y / 8 * 8) % 16 < 8)
                data[y * width + x] = 0x9800000;
            else
                data[y * width + x] = 0xFEEEEEE;
            */
        }
    }
...
main(int argc, char *argv[])
{
    ...
    xdg_toplevel_set_title(state.xdg_toplevel, "Exampwale client");
    wl_surface_commit(state.wl_surface);
